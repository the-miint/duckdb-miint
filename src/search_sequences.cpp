...
// `read_id` mirrors the query input's read_id type; `target_id` mirrors the
// reference (db) input's read_id type. Each may independently be VARCHAR or
// BIGINT. Other columns are fixed.
static std::vector<LogicalType> GetSearchOutputTypes(const LogicalType &query_id_type, const LogicalType &ref_id_type) {
	...
	return {query_id_type,        ref_id_type,          LogicalType::DOUBLE,  LogicalType::INTEGER,
	        LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER,
	        LogicalType::INTEGER, LogicalType::BOOLEAN};
}

// Write a batch of SearchResults into a DataChunk. The two id columns
// (`read_id`, `target_id`) dispatch on the bind-captured types via the
// shared EmitIdColumnFromStrings helper.
static idx_t OutputSearchResults(DataChunk &output, const std::vector<miint::SearchResult> &results, idx_t offset,
                                 idx_t count, const LogicalType &query_id_type, const LogicalType &ref_id_type) {
	...
	// Build the two id-string projections once per chunk; emit via the codec.
	std::vector<std::string> query_ids(actual);
	std::vector<std::string> target_ids(actual);
	for (idx_t i = 0; i < actual; i++) {
		query_ids[i] = results[offset + i].query_id;
		target_ids[i] = results[offset + i].target_id;
	}
	EmitIdColumnFromStrings(output.data[col++], query_ids, /*offset=*/0, actual, query_id_type);
	EmitIdColumnFromStrings(output.data[col++], target_ids, /*offset=*/0, actual, ref_id_type);
...
unique_ptr<FunctionData> SearchSequencesTableFunction::Bind(ClientContext &context, TableFunctionBindInput &input,
	...
	// query input type, output target_id mirrors the reference input type.
...
unique_ptr<GlobalTableFunctionState> SearchSequencesTableFunction::InitGlobal(ClientContext &context,
	...
	auto ref = LoadSingleEndSequences(context, data.ref_table, "search_sequences_vsearch", data.ref_schema);
