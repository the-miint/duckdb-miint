...
RypeClassifyTableFunction::GlobalState::~GlobalState() {
	...
	// Then the input stream: RYpe consumes it lazily, so it must stay alive until
	// output_stream.release above has returned. Its release callback has already
	// dropped the scan by now; this frees the shell.
	input_stream.reset();

	// Release sub-connection LAST — the input stream holds a streaming QueryResult
	// that has a non-owning optional_ptr<ClientContext> into this connection.
...
unique_ptr<GlobalTableFunctionState> RypeClassifyTableFunction::InitGlobal(ClientContext &context,
	...
	// RYpe lazily consumes the input Arrow stream, which owns a streaming
	// QueryResult holding a non-owning optional_ptr<ClientContext>. If the
	// connection is destroyed before RYpe finishes consuming, the dangling pointer
	...
	// only once it starts consuming, and a -1 return does not distinguish that
	// from failing before — see the ownership note in rype_input_stream.hpp. Since
	// nothing here hands the object over, both outcomes are handled by the
	// destructor and neither can double-free.
	int result = rype_classify_arrow_ex(gstate->index, gstate->negative_set, &gstate->input_stream->stream,
	                                    bind_data.threshold, batch_size, &gstate->output_stream);
...
void RypeClassifyTableFunction::Execute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	for (idx_t i = 0; i < to_output; i++) {
		...
		// The surrogate is only ever emitted by RypeInputStream after it has
		// recorded the matching identifier, so any out-of-range value means RYpe
