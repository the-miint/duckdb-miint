...
std::shared_ptr<Bowtie2ActiveShard> AlignBowtie2ShardedTableFunction::ClaimWork(ClientContext &context,
	...
	// Phase 4: Materialize read IDs and pre-fetch sequences OUTSIDE lock.
	...
	// Phases 4a+4b: Materialize read IDs and pre-fetch sequences.
	...
	try {
		...
		// Phase 4b: Pre-fetch ALL sequences for this shard in one bulk query.
		...
		SHARD_DBG_MEM(gstate, "ClaimWork: PRE-FETCHED %zu sequences for shard %zu in %ldms",
...
void AlignBowtie2ShardedTableFunction::Execute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	while (true) {
		...
		// 3. Atomically claim batch offset into the pre-fetched sequence list
		...
		if (my_offset >= seq_count) {
			...
			SHARD_DBG(global_state, "Execute: shard %zu offset %zu >= seq_count %zu, finishing subprocess",
		...
		// 4. Clamp batch count to remaining sequences
		idx_t batch_count = std::min(SHARD_READ_BATCH_SIZE, seq_count - my_offset);
		...
		// 5. Extract sub-batch from pre-fetched sequences (no per-batch SQL round-trip)
