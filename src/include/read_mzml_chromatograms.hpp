...
namespace duckdb {
class ReadMzMLChromatogramsTableFunction {
public:
	struct Data : public TableFunctionData {
		...
		Data(const std::vector<std::string> &paths, bool include_fp)
		    : file_paths(paths), include_filepath(include_fp),
		      names({"chromatogram_index", "chromatogram_id", "chromatogram_type", "precursor_mz", "product_mz",
		             "time_array", "intensity_array"}),
		      types({LogicalType::INTEGER, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE,
