...
Returns a table of the pinned versions of miint and every embedded library, one row per component. Which rows appear depends on the build's compile-time features (HDF5, vsearch, mafft, abpoa, sylph, st3, krepp, unifrac, libcurl, libdeflate are conditional). The `krepp-openmp` row, whose version is `enabled`, appears only when krepp's OpenMP regions are compiled in, which is what lets [`krepp_index_create`](phylogeny.md#build-a-krepp-index) accept `threads` > 1.

**Output schema:**

| Column | Type | Description |
|--------|------|-------------|
| `library` | VARCHAR | Component name (`miint`, `htslib`, `minimap2`, `kseq++`, `LBFGS++`, `WFA2-lib`, `zlib`, `rype`, and conditionally `HDF5`, `libdeflate`, `libcurl`, `vsearch`, `mafft`, `abpoa`, `sylph`, `st3`, `krepp`, `krepp-openmp`, `unifrac`, `scikit-bio-binaries`) |
