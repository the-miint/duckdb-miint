# UniFrac Analysis Functions

Three table functions implementing the standard UniFrac-based microbiome diversity workflow on long-form feature tables and rooted phylogenies — no Python round-trip required.

| Function | What it computes |
|---|---|
| [`unifrac_pcoa`](#unifrac_pcoa) | UniFrac distance + Principal Coordinates Analysis (FSVD-based; randomized fp32) |
| [`unifrac_permanova`](#unifrac_permanova) | UniFrac distance + PERMANOVA pseudo-F + p-value for one or more grouping variables |
| [`unifrac_faith_pd`](#unifrac_faith_pd) | Faith's phylogenetic diversity per sample |

All three are powered by the embedded [`unifrac-binaries`](https://github.com/biocore/unifrac-binaries) and [`scikit-bio-binaries`](https://github.com/scikit-bio/scikit-bio-binaries) libraries (see `docs/internals/embedded-tools.md` for the build details).

Availability depends on the build:

```sql
SELECT version FROM miint_versions() WHERE library IN ('unifrac', 'scikit-bio-binaries');
```

Returns empty when the extension was built with `MIINT_ENABLE_UNIFRAC=OFF` (auto-disabled on Windows).

## Common inputs

All three functions read **relations** (tables or views) by name, not file paths. The relations are read at bind time via a separate `Connection` and validated before compute begins.

### Feature table

A long-form `(sample_id VARCHAR, feature_id VARCHAR, value DOUBLE)` relation — the same schema produced by [`read_biom`](table-functions.md#read_biom). Duplicate `(sample_id, feature_id)` rows are summed; rows with NULL or zero values are silently dropped (sparse-storage invariant).
...
A relation matching the [`read_newick`](table-functions.md#read_newick) schema: `(node_index BIGINT, parent_index BIGINT, name VARCHAR, branch_length DOUBLE, edge_id BIGINT)`. The tree's tip names must include every `feature_id` present in the feature table — otherwise binding fails with an error naming the missing tip.

```sql
CREATE TABLE tree AS SELECT * FROM read_newick('data/unifrac/gg_otu_tree.nwk');
```

### Metadata (PERMANOVA only)
...
- `variant` (VARCHAR, default `'weighted_normalized'`): one of the [variants](#variant-strings) above
...
- `subsample_depth`, `subsample_with_replacement`, `n_subsamples`, `seed`: see [Subsampling](#subsampling-rarefaction)
...
- `metadata` (VARCHAR): wide-form metadata relation name (see [Metadata](#metadata-permanova-only))
...
- [`read_biom`](table-functions.md#read_biom) — produces the long-form feature-table relation these functions consume
- [`read_newick`](table-functions.md#read_newick) — produces the tree relation
- [`miint_versions`](table-functions.md#miint_versions) — reports the pinned versions of `unifrac` and `scikit-bio-binaries`
- [`docs/internals/embedded-tools.md`](internals/embedded-tools.md) — submodule pins, build configuration, native vs WASM divergence
