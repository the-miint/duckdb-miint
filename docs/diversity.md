...
- <a name="progressive-unifrac-batch-size"></a>**Tune `m = n_anchors + batch_size`, and keep it small — the opposite of what a serial run wants.** A block's cost grows with `m` faster than linearly, and once blocks run concurrently it is total CPU, not per-block latency, that sets wall time; smaller blocks also mean *more* of them, which is what there is to parallelize. Serially the ordering reverses (big blocks amortize a fixed per-block cost), so a `batch_size` tuned on a one-core run is the wrong one here. The optimum tracks `m`, not `batch_size` alone — at `n_anchors := 200`, `batch_size` 244 → 31 s, 500 → 34 s, 1000 → 56 s, 1849 → 73 s, 3000 → 103 s (17,483 samples, `threads := 14`, total CPU rising 3.5× across the range); at `n_anchors := 1000` the same table's optimum moves out to `batch_size` 500–1000, because `n_anchors` then dominates `m` and shrinking `batch_size` only adds blocks without shrinking the block.
