...
- <a name="progressive-unifrac-threads"></a>**`threads` controls block CONCURRENCY here, and it is the main performance knob.** Unlike `unifrac_pcoa`, a single block cannot use more than one core no matter how high you set it: libssu's UniFrac parallel degree is `ceil(n_samples / 2048)` over the *block*, and a block is only `n_anchors + batch_size` samples, so at any ordinary setting that is one stripe. So `threads` is spent running that many *blocks* at once instead, each pinned narrow. Measured on 17,483 samples (`n_dims := 5`, `n_anchors := 200`, `batch_size := 500`): **250 s at `threads := 1` → 32 s at `threads := 14`**, 1.1 → 9.8 cores busy.
- <a name="progressive-unifrac-batch-size"></a>**Prefer a SMALL `batch_size` here — the opposite of what a serial run wants.** A block's cost grows with `m = n_anchors + batch_size` faster than linearly, and once blocks run concurrently it is total CPU, not per-block latency, that sets wall time. Smaller blocks also mean *more* of them, which is what there is to parallelize. Same table as above at `threads := 14`: `batch_size` 244 → 31 s, 500 → 34 s, 1000 → 56 s, 1849 → 73 s, 3000 → 103 s, with total CPU rising 3.5× across that range. (Serially the ordering reverses — big blocks amortize a fixed per-block cost — so a `batch_size` tuned on a one-core run is the wrong one here.) `batch_size` does change coordinates, so pin it alongside the seed when comparing runs.
- **Reproducibility is to ~1e-5, not bit-exact, whenever blocks run concurrently.** Two effects, both fp32-noise scale and neither a correctness bug. (1) `threads` also sets the OpenMP width of the *anchor* block's reference PCoA, and skbb's centering is an OpenMP `reduction(+:)` whose summation order depends on that width — so the reference frame shifts in its last bits and every coordinate inherits the shift (measured 3.1e-06 between `threads := 1` and `threads := 4`). (2) Concurrent *first* calls race on libssu/skbb's non-atomic CPU/accelerator detection caches, which perturbs the **first** progressive run in a session by ~1.6e-06 and nothing after it. `threads := 1` is bit-exact run-to-run. If you need bit-identical coordinates, fix `threads := N` alongside the seed and re-run within one session, or use `threads := 1`; see [Reproducibility](#reproducibility) for the same fp32 caveat on the non-progressive paths.
